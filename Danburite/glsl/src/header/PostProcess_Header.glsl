...
sampler2DRect PostProcess_getRectColorAttachment(const uint idx)
{
	uvec2 handle;
...
vec4 PostProcess_getRectPixel(const uint colorAttachmentIdx, const vec2 texCoord)
{
	return texture(PostProcess_getRectColorAttachment(colorAttachmentIdx), texCoord);
}
